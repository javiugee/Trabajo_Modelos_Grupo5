...
modelo_lineal <- lm(G3 ~ G1 + G2 +famrel + absences + age +  Walc, data = df_mat) ### df_mat --- train
car::crPlots(modelo_lineal)
```
En cuanto a la no linealidad para estas variables, para "G1", "G2", "famrel", "age" y "Walc" se puede ver que la línea rosa está prácticamnenete superpuesto a la línea azul, lo que indica que existe una relación de linealidad.
...
Si $\lambda = 1$, no hay transformación.
Si $\lambda = 0.5$, transformación de raíz cuadrada.
Si $\lambda = 0$, transformación logarítmica.
Si $\lambda = -1$, transformación inversa.
...
Como nos ha salido que $\lambda = 1.2727$, al estar el valor cerca del uno, quiere decir que no se tiene que llevar a cabo ninguna transformación.